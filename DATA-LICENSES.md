...
The **source code** of Brahmaand is MIT-licensed (see [LICENSE](LICENSE)) — that license covers the
original source code only. The astronomical **data and imagery** bundled here, and data fetched at
runtime from third parties, are *not* MIT — they belong to their providers and carry their own terms.
This file is the required attribution. Keep it with any copy, redistribution or deployment, and
surface the same credits in the app's UI (the in-app panels already do).
...
| `data-src/hyg.csv` (raw HYG v4.1 catalog; build input for `hyg.bin` via `tools/build-stars.mjs`) | [HYG database](https://github.com/astronexus/HYG-Database), astronexus | **CC BY-SA 4.0** — attribution + share-alike. Redistributed unmodified as the pipeline source so the star catalog is reproducible from a clean clone. |
